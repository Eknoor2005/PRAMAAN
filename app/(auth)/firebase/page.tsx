...
export default function FirebaseAuthPage() {
  ...
  const handleEmailAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isSignUp) {
        ...
        // Send user data to backend to create PRAMAAN account
        ...
        if (!response.ok) {
          throw new Error('Failed to create PRAMAAN account');
        }
      } else {
  ...
  const handleSocialAuth = async (provider: typeof googleProvider | typeof githubProvider, providerName: string) => {
    ...
    try {
      ...
      const response = await fetch('/api/auth/firebase/social', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          Authorization: `Bearer ${idToken}`,
      ...
      if (!response.ok) {
        throw new Error('Failed to authenticate with PRAMAAN');
      }
